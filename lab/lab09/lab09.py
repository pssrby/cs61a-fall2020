...
#    return subseq_helper(s, -1)
    if not s:
        return [[]]
    a = inc_subseqs(s[1:])
    b = [lst for lst in a if not lst or s[0] <= lst[0]]
    return insert_into_all(s[0], b) + a
...
def num_trees(n):
    ...
    *   *        *       *
    ...
    if n == 1:
        return 1
#    return 2 * sum([num_trees(i) * num_trees(n - i) for i in range(1, (n + 1) // 2)]) \
#            + (0 if n % 2 else num_trees(n // 2) * num_trees(n // 2))
...
class Keyboard:
    ...
    def press(self, info):
        ...
        if button := self.buttons.get(info):
...
def insert(link, value, index):
    ...
    if index == 0:
        ...
        link.first = value
        None
    elif index > 0 and not link.rest is Link.empty:
...
def prune_small(t, n):
    ...
    """
    while n < len(t.branches) and t.branches != []:
