...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        for deck_card in player.deck.cards:
            deck_card.attack += other_card.attack
            deck_card.defense += other_card.defense
        length = orig_opponent_deck_length
        index = 0
        while index < length:
            deck_card = opponent.deck.cards[index]
            if deck_card.attack == other_card.attack or deck_card.defense == other_card.defense:
